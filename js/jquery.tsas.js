...
(function($){
	...
	$.extend($.fn, {
		...
		initDatePanel: function(setting){
			...
			$.tsas.step = ($dateBox.width() / $.tsas.displayCount).toFixed(4);//砖块步长,必须截取小数点位数,不然会存在砖块像素误差
			
			window.$animate = $mainBox.animateEffect(setting);
			...
			if(dateArray != null && dateArray.length > 0){
				...
				$datePanel.width(bw);
				$.BindingUtils.bindCss($brickPanel,"width",$datePanel,"width");
				
				var dataBriks = "";
				for(var i=0;i<dateArray.length;i++){
					var dateBrick = new DateBrick({
		...
		initRolePanel: function(setting){
			...
			$rolePanel.find("div").contextMenu({
				data:[{"value":1,"text":"新增角色"},{"value":2,"text":"修改角色"},{"value":3,"text":"删除角色"}],
				callback:function(obj,v,t){
					var options = obj.data("options");
					switch(v){
						case 1:
						  	setting.templates.insertRole.id.renderInsertRolePanel(setting,options);
						  	break;
						case 2:
						  	setting.templates.updateRole.id.renderUpdateRolePanel(setting,options);
						  	break;
						case 3:
						  	setting.templates.brickLock.id.renderBrickLockPanel(setting,options);
						  	break;
						default:
							setting.templates.brickDelete.id.renderBrickDeletePanel(setting,options);
					}
				}
			});
		},
		...
		bindDrag:function(setting){
			...
			var boxEvent = {
                drag: function(e) {
                	//$roleBox.text("scrollLeft:"+$brickBox[0].scrollLeft);
                	
					var px = $brickBox.scrollLeft() + point.x - e.pageX;
					var py = $brickBox.scrollTop() + e.pageY - point.y;
					point.x = e.pageX;
					point.y = e.pageY;
					var maxW = ($datePanel.width()-$mainBox.width());
					
					$brickBox.scrollLeft(Math.min(px,maxW));
					$dateBox.scrollLeft(Math.min(px,maxW));
					
					$brickBox.scrollTop(py);
					$roleBox.scrollTop(py);
					
					/*if ($dateBox.scrollLeft() > maxW){
                ...
                drop: function(e) {
					...
					if(c != null && n != null){
						...
						var left = parseFloat(brick.attr("x")).toFixed(0);//必须四舍五入取整,否则会存在像素误差
		...
		initBrickPanel: function(setting){
			...
			$brickPanel.find("g").contextMenu({
				...
				callback:function(obj,v,t){
					...
					switch(v){
						case 1:
						  	setting.templates.brickInfo.renderBrickInfoPanel(setting,options);
						  	break;
						case 2:
						  	setting.templates.brickProperty.id.renderBrickPropertyPanel(setting,options);
						  	break;
						case 3:
						  	setting.templates.brickLock.id.renderBrickLockPanel(setting,options);
						  	break;
						case 4:
						  	setting.templates.insertBrick.id.renderInsertBrickPanel(setting,options);
						  	break;
						case 5:
						  	setting.templates.insertBrick.id.renderInsertBrickPanel(setting,options);
						  	break;
		...
		renderBrickInfoPanel : function(setting,options){
			$.overlay();
			var $this = $(this);
			var leaf = options.leaf;
			leaf.consume = options.consume;
			leaf.storageTypeText = options.storageTypeText;
			leaf.main = options.main ? "是" : "否";
			leaf.beginDate = options.beginDate;
			leaf.endDate = options.endDate;
			try{
				$this.tmpl(leaf).prependTo("body").find(":button.cancel").click(function(){
					$.removeOverlay();
					$(this).closest(".dialog").remove();
				});
			}catch(e){
				$.MsgUtils.log("渲染烟叶信息面板异常!");
			}
			
		},
		renderBrickPropertyPanel : function(setting,options){
			$.overlay();
			var $this = $(this);
			var leaf = options.leaf;
			leaf.consume = options.consume;
			...
			try{
				...
				$.MsgUtils.log("渲染修改烟叶属性面板异常!");
			}
			
			var $isMain = $(setting.templates.brickProperty.form.isMain).prop("checked",options.main);
		},
		renderBrickLockPanel : function(setting,options){
			$.overlay();
			var $this = $(this);
			var leaf = options.leaf;
			leaf.consume = options.consume;
		...
		renderBrickDeletePanel : function(setting,options){
			$.overlay();
			var $this = $(this);
			var leaf = options.leaf;
			leaf.consume = options.consume;
			...
			try{
				...
				$.MsgUtils.log("渲染烟叶删除面板异常!");
			}
			var d = [{"id":1,"ksrq":"2014-09-01","jsrq":"2014-09-30","syl":100,"sfsd":"是"}];
			var $txtStartDate = $(setting.templates.brickDelete.form.startDate).datebox();
		...
		renderInsertRolePanel : function(setting){
			$.overlay();
			var $this = $(this);
			$this.tmpl(null).prependTo("body").find(":button.cancel").click(function(){
				$.removeOverlay();
			...
			var $leafTab = $(setting.templates.insertRole.tabs.leafTab);
			$leafTab.datagrid({
				idField:'id',
				treeField:'name',
				pagination:true,
		    ...
		    var $bundleTab = $(setting.templates.insertRole.tabs.bundleTab);
		    $bundleTab.datagrid({
				idField:'id',
				treeField:'name',
				pagination:false,
			...
			var $qualityTab = $(setting.templates.insertRole.tabs.qualityTab);
		    $qualityTab.datagrid({
				idField:'id',
				treeField:'name',
				pagination:false,
			...
			var $leafUseTab = $(setting.templates.insertRole.tabs.leafUseTab);
		    $leafUseTab.datagrid({
				idField:'id',
				treeField:'name',
				pagination:false,
		...
		renderUpdateRolePanel : function(setting,options){
			$.overlay();
			var $this = $(this);
			//var leaf = options.leaf;
			//leaf.consume = options.consume;
			...
			var d = [{"id":1,"ksrq":"2014-09-01","jsrq":"2014-09-30","syl":100,"sfsd":"是"}];
			var $txtStartDate = $(setting.templates.updateRole.form.startDate).datebox();
			var $leafUseTab = $(setting.templates.updateRole.tabs.leafUseTab);
		    $leafUseTab.datagrid({
		    	data:d,
				idField:'id',
				treeField:'name',
		...
		animateEffect : function(setting){
			var $this = $(this);
			var $leftBox = setting.boxes.leftBox;
			/*    动画1  */
			$leftBox.css({"left":-$leftBox.width()});
			$this.css({"right":-$this.width()});
			return {
				start : function(){
					$this.animate({"right":0},1000);
					$leftBox.animate({"left":0},1000);
				}
			};
			
			/*    动画2
			var $box = setting.templates.animateTemplate.one.tmpl(null).css("left",$this.scrollLeft()).prependTo($this);
			return {
				start : function(){
					$box.find(".left-up").animate({"width":0,"height":0},2000,"linear");
					$box.find(".right-up").animate({"width":0,"height":0},2000,"linear");
					$box.find(".left-down").animate({"width":0,"height":0},2000,"linear");
					$box.find(".right-down").animate({"width":0,"height":0},2000,"linear");
				}
			};*/
			
			
			
			//var $box = $('<div style="width:100%;height:100%;position:absolute;z-index:999999999;"></div>').css("left",$this.scrollLeft()).prependTo($this);
			//animateTemplate
			/*var x = $this.width() / 2 + $this.scrollLeft();
			var y = $this.height() / 2;
			var animateCircle = new SVGCircle();
			...
			//view.children.push(animateCircle.render());
			...
			var handler = function(){
				...
				var r = 700-count;
				$("#animateCircle").attr("r",700-count);
			};
			var timer = new CustomerTimer(1,handler);
			//timer.start();*/
		},
		initBrickColor : function(){
			...
			colorMap.put($.tsas.brickColorIdPrefix+$.tsas.brickTypeIds.lock,$.tsas.colors.lock);
				...
				if(e.which == 3){
					...
					if(ps.data){
						...
						$menu.find("li").mousemove(function(){
							...
							var $li = $(this);
							if(ps.callback){
								ps.callback($this,$li.data("value"),$li.text());
							}
							$(".context-menu","body").remove();
						});
